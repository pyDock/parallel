...
type ftdock >/dev/null 2>&1 || { echo >&2 "FTDock not found check the PATH variable or install the program. Aborting."; exit ${E_BADARGS}; }
is_MPI=`ftdock | grep "MPI" | wc -l`
if [ $is_MPI -eq 0 ]
    then
        echo "ftdock MPI version not found. Please, check PATH.";
        exit ${E_BADARGS};
fi
if [ "$#" != "3" ] && [ "$#" != "2" ]; then
    echo "Wrong arguments";
    printf "Usage: `basename $0` project_name num_cpus noelec\nThe default setting of ftdock is perform the sampling with electrostatics to desactivate include the \"nonelc\" parameter.\n";
    exit ${E_BADARGS};
else
    int='^[0-9]+$'
    if ! [[ $2 =~ $int ]];then
...
if [ "$3" == "noelec" ];then
	...
	mpirun -np $2 ftdock -static ${1}_rec.pdb -mobile ${1}_lig.pdb -calculate_grid 0.7 -angle_step 12 -internal -15 -surface 1.3 -keep 3 -noelec -out ${1}.ftdock
else
	echo "Executing ftdock MPI with electrostatic"
	mpirun -np $2 ftdock -static ${1}_rec.pdb -mobile ${1}_lig.pdb -calculate_grid 0.7 -angle_step 12 -internal -15 -surface 1.3 -keep 3 -out ${1}.ftdock
fi;
